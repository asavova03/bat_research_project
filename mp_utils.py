...
def plot_dictionary_elements(dictionary):
    ...
    for i, ax in enumerate(axes.flat):
        if i < len(dictionary):
            ...
            ax.plot(kernel, linewidth=50.0/kernel_length + 0.1, color='deepskyblue')
            
            # Add some text indicating the length of the kernel
            # ax.text(0.5, 0.9, f"{kernel_length} samples", fontsize=8, color='orangered', transform=ax.transAxes)
